...
class MyCentralManager extends CentralController {
  ...
  Future<List<GattService>> getServices(Peripheral peripheral) async {
    ...
    return blueZDevice.gattServices.map((service) {
      final instance = _myServices[service.hashCode];
      return instance is MyGattService ? instance : MyGattService(service);
    }).toList();
  }
  ...
  Future<List<GattCharacteristic>> getCharacteristics(
    ...
    return blueZService.characteristics.map((characteristic) {
      ...
      return instance is MyGattCharacteristic
          ? instance
  ...
  Future<List<GattDescriptor>> getDescriptors(
    ...
    return blueZCharacteristic.descriptors.map((descriptor) {
      final instance = _myDescriptors[descriptor.hashCode];
      return instance is MyGattDescriptor
          ? instance
  ...
  void _onDiscovered(BlueZDevice device) {
    final instance = _myPeripherals[device.hashCode];
    final myPeripheral =
        instance is MyPeripheral ? instance : MyPeripheral(device);
    final rssi = device.rssi;
    final advertisement = device.advertisement;
    final eventArgs =
        CentralDiscoveredEventArgs(myPeripheral, rssi, advertisement);
