...
enum class ConnectionPriorityArgs(val raw: Int) {
  BALANCED(0),
  HIGH(1),
  LOW_POWER(2);

  companion object {
    fun ofRaw(raw: Int): ConnectionPriorityArgs? {
      return values().firstOrNull { it.raw == raw }
    }
  }
}
...
private open class BluetoothLowEnergyApiPigeonCodec : StandardMessageCodec() {
  override fun readValueOfType(type: Byte, buffer: ByteBuffer): Any? {
    return when (type) {
      ...
      133.toByte() -> {
        return (readValue(buffer) as Long?)?.let {
          ConnectionPriorityArgs.ofRaw(it.toInt())
        }
      }
      134.toByte() -> {
        return (readValue(buffer) as Long?)?.let {
...
interface CentralManagerHostApi {
  ...
  fun requestConnectionPriority(addressArgs: String, priorityArgs: ConnectionPriorityArgs)
      ...
      run {
        ...
        if (api != null) {
          channel.setMessageHandler { message, reply ->
            val args = message as List<Any?>
            val addressArgsArg = args[0] as String
            val priorityArgsArg = args[1] as ConnectionPriorityArgs
            val wrapped: List<Any?> = try {
