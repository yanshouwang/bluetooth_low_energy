  ...
  fun onDiscovered(peripheralArgsArg: PeripheralArgs, rssiArgsArg: Long, advertisementArgsArg: AdvertisementArgs, callback: (Result<Unit>) -> Unit)
{
  ...
  fun onConnectionStateChanged(peripheralArgsArg: PeripheralArgs, statusArgsArg: Long, stateArgsArg: ConnectionStateArgs, callback: (Result<Unit>) -> Unit)
{
    val separatedMessageChannelSuffix = if (messageChannelSuffix.isNotEmpty()) ".$messageChannelSuffix" else ""
    ...
    val channel = BasicMessageChannel<Any?>(binaryMessenger, channelName, codec)
    channel.send(listOf(peripheralArgsArg, statusArgsArg, stateArgsArg)) {
      if (it is List<*>) {
        if (it.size > 1) {
          callback(Result.failure(FlutterError(it[0] as String, it[1] as String, it[2] as String?)))
        } else {
