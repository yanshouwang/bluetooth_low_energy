...
{
	PeripheralManagerImpl::PeripheralManagerImpl(flutter::BinaryMessenger *messenger)
	{
		const auto api = PeripheralManagerFlutterApi(messenger);
	...
	void PeripheralManagerImpl::Initialize(std::function<void(std::optional<FlutterError> reply)> result)
	{
		InitializeAsync(std::move(result));
	}

	ErrorOr<BluetoothLowEnergyStateArgs> PeripheralManagerImpl::GetState()
	{
		try
		{
	...
	void PeripheralManagerImpl::AddService(const MutableGATTServiceArgs &service_args, std::function<void(std::optional<FlutterError> reply)> result)
	{
		AddServiceAsync(service_args, std::move(result));
	}

	std::optional<FlutterError> PeripheralManagerImpl::RemoveService(int64_t hash_code_args)
	{
		try
		{
	...
	std::optional<FlutterError> PeripheralManagerImpl::StartAdvertising(const AdvertisementArgs &advertisement_args)
	{
		try
		{
			...
			const auto &service_uuids_args = advertisement_args.service_u_u_i_ds_args();
			...
			const auto &service_data_args = advertisement_args.service_data_args();
			for (const auto &data_args_value : service_data_args)
			{
				...
				const auto data_writer = winrt::Windows::Storage::Streams::DataWriter();
				const auto &uuid_args = std::get<std::string>(data_args_value.first);
				const auto uuid = winrt::guid(uuid_args);
				data_writer.WriteGuid(uuid);
				const auto &data_args = std::get<std::vector<uint8_t>>(data_args_value.second);
				data_writer.WriteBytes(data_args);
				const auto data = data_writer.DetachBuffer();
			...
			for (const auto &data_args_value : manufacturer_specific_data_args)
			{
				const auto &custom_data_args_value = std::get<flutter::CustomEncodableValue>(data_args_value);
				const auto data_args = std::any_cast<ManufacturerSpecificDataArgs>(custom_data_args_value);
				const auto data = ArgsToManufacturerData(data_args);
	...
	std::optional<FlutterError> PeripheralManagerImpl::StopAdvertising()
	{
		try
		{
			const auto &publisher = m_publisher.value();
	...
	ErrorOr<int64_t> PeripheralManagerImpl::GetMaxNotificationSize(int64_t address_args)
	{
		try
		{
	...
	std::optional<FlutterError> PeripheralManagerImpl::RespondReadRequestWithValue(int64_t id_args, const std::vector<uint8_t> &value_args)
	{
		try
		{
	...
	std::optional<FlutterError> PeripheralManagerImpl::RespondReadRequestWithProtocolError(int64_t id_args, const GATTProtocolErrorArgs &error_args)
	{
		try
		{
			const auto &deferal = m_deferrals[id_args].value();
	...
	std::optional<FlutterError> PeripheralManagerImpl::RespondWriteRequest(int64_t id_args)
	{
		try
		{
			const auto &deferal = m_deferrals[id_args].value();
	...
	std::optional<FlutterError> PeripheralManagerImpl::RespondWriteRequestWithProtocolError(int64_t id_args, const GATTProtocolErrorArgs &error_args)
	{
		try
		{
			const auto &deferal = m_deferrals[id_args].value();
	...
	void PeripheralManagerImpl::NotifyValue(int64_t address_args, int64_t hash_code_args, const std::vector<uint8_t> &value_args, std::function<void(std::optional<FlutterError> reply)> result)
	{
		NotifyValueAsync(address_args, hash_code_args, value_args, std::move(result));
	}

	winrt::fire_and_forget PeripheralManagerImpl::InitializeAsync(std::function<void(std::optional<FlutterError> reply)> result)
	{
		try
		{
			const auto &publisher = m_publisher.value();
	...
	winrt::fire_and_forget PeripheralManagerImpl::AddServiceAsync(const MutableGATTServiceArgs &service_args, std::function<void(std::optional<FlutterError> reply)> result)
	{
		try
		{
	...
	winrt::fire_and_forget PeripheralManagerImpl::NotifyValueAsync(int64_t address_args, int64_t hash_code_args, const std::vector<uint8_t> &value_args, std::function<void(std::optional<FlutterError> reply)> result)
	{
		try
		{
			const auto &client = m_clients[address_args].value();
	...
	winrt::Windows::Foundation::IAsyncAction PeripheralManagerImpl::CreateServiceAsync(const MutableGATTServiceArgs service_args)
	{
		...
		{
			const auto error_code = static_cast<int>(error);
		...
		const auto &characteristics_args = service_args.characteristics_args();
		for (const auto &characteristic_args_value : characteristics_args)
		{
			const auto &custom_characteristic_args_value = std::get<flutter::CustomEncodableValue>(characteristic_args_value);
			const auto characteristic_args = std::any_cast<MutableGATTCharacteristicArgs>(custom_characteristic_args_value);
	...
	winrt::Windows::Foundation::IAsyncAction PeripheralManagerImpl::CreateCharacteristicAsync(const winrt::Windows::Devices::Bluetooth::GenericAttributeProfile::GattLocalService &service, const MutableGATTCharacteristicArgs characteristic_args)
	{
		...
		const auto &property_numbers_args = characteristic_args.property_numbers_args();
		const auto properties = ArgsToCharacteristicProperties(property_numbers_args);
		...
		if (write_protection_level_args)
		{
		...
		const auto &descriptors_args = characteristic_args.descriptors_args();
		for (const auto &descriptor_args_value : descriptors_args)
		{
			const auto &custom_descriptor_args_value = std::get<flutter::CustomEncodableValue>(descriptor_args_value);
			const auto descriptor_args = std::any_cast<MutableGATTDescriptorArgs>(custom_descriptor_args_value);
			co_await CreateDescriptorAsync(characteristic, descriptor_args);
		}
		m_characteristics[hash_code_args] = characteristic;
	}

	winrt::Windows::Foundation::IAsyncAction PeripheralManagerImpl::CreateDescriptorAsync(const winrt::Windows::Devices::Bluetooth::GenericAttributeProfile::GattLocalCharacteristic &characteristic, const MutableGATTDescriptorArgs descriptor_args)
	{
	...
	void PeripheralManagerImpl::RemoveServiceArgs(const MutableGATTServiceArgs &service_args)
	{
		const auto &characteristics_args = service_args.characteristics_args();
		for (const auto &characteristic_args_value : characteristics_args)
		{
	...
	void PeripheralManagerImpl::RemoveCharacteristicArgs(const MutableGATTCharacteristicArgs &characteristic_args)
	{
		const auto hash_code_args = characteristic_args.hash_code_args();
		const auto &descriptors_args = characteristic_args.descriptors_args();
		for (const auto &descriptor_args_value : descriptors_args)
	...
	void PeripheralManagerImpl::RemoveDescriptorArgs(const MutableGATTDescriptorArgs &descriptor_args)
	{
		const auto hash_code_args = descriptor_args.hash_code_args();
	...
	winrt::fire_and_forget PeripheralManagerImpl::OnCharacteristicReadRequestedAsync(const int64_t hash_code_args, const winrt::Windows::Devices::Bluetooth::GenericAttributeProfile::GattReadRequestedEventArgs event_args)
	{
		auto &api = m_api.value();
		...
		const auto central_args = CentralArgs(address_args);
		...
		const auto request_args = GATTReadRequestArgs(id_args, offset_args, length_args);
		api.OnCharacteristicReadRequest(central_args, hash_code_args, request_args, []() {}, [](const auto &error) {});
	}

	winrt::fire_and_forget PeripheralManagerImpl::OnCharacteristicWriteRequestedAsync(const int64_t hash_code_args, const winrt::Windows::Devices::Bluetooth::GenericAttributeProfile::GattWriteRequestedEventArgs event_args)
	{
		auto &api = m_api.value();
		const auto deferral = event_args.GetDeferral();
		const auto session = event_args.Session();
		...
		const auto request_args = GATTWriteRequestArgs(id_args, offset_args, value_args, type_args);
		api.OnCharacteristicWriteRequest(central_args, hash_code_args, request_args, []() {}, [](const auto &error) {});
	}

	winrt::fire_and_forget PeripheralManagerImpl::OnCharacteristicSubscribedClientsChangedAsync(const int64_t hash_code_args, const winrt::Windows::Devices::Bluetooth::GenericAttributeProfile::GattLocalCharacteristic &characteristic)
	{
		auto &api = m_api.value();
	...
	winrt::fire_and_forget PeripheralManagerImpl::OnDescriptorReadRequestedAsync(const int64_t hash_code_args, const winrt::Windows::Devices::Bluetooth::GenericAttributeProfile::GattReadRequestedEventArgs event_args)
	{
		auto &api = m_api.value();
		const auto deferral = event_args.GetDeferral();
		const auto session = event_args.Session();
	...
	winrt::fire_and_forget PeripheralManagerImpl::OnDescriptorWriteRequestedAsync(const int64_t hash_code_args, const winrt::Windows::Devices::Bluetooth::GenericAttributeProfile::GattWriteRequestedEventArgs event_args)
	{
		auto &api = m_api.value();
		const auto deferral = event_args.GetDeferral();
		const auto session = event_args.Session();
	...
	BluetoothLowEnergyStateArgs PeripheralManagerImpl::RadioStateToArgs(const winrt::Windows::Devices::Radios::RadioState &state)
	{
		switch (state)
		{
	...
	winrt::Windows::Devices::Bluetooth::Advertisement::BluetoothLEManufacturerData PeripheralManagerImpl::ArgsToManufacturerData(const ManufacturerSpecificDataArgs &manufacturer_specific_data_args)
	{
		const auto manufacturer_data = winrt::Windows::Devices::Bluetooth::Advertisement::BluetoothLEManufacturerData();
		const auto id_args = manufacturer_specific_data_args.id_args();
		const auto id = static_cast<uint16_t>(id_args);
		manufacturer_data.CompanyId(id);
	...
	winrt::Windows::Devices::Bluetooth::GenericAttributeProfile::GattCharacteristicProperties PeripheralManagerImpl::ArgsToCharacteristicProperties(const flutter::EncodableList property_numbers_args)
	{
	...
	GATTCharacteristicWriteTypeArgs PeripheralManagerImpl::WriteOptionToArgs(const winrt::Windows::Devices::Bluetooth::GenericAttributeProfile::GattWriteOption &option)
	{
		switch (option)
		{
		case winrt::Windows::Devices::Bluetooth::GenericAttributeProfile::GattWriteOption::WriteWithResponse:
			return GATTCharacteristicWriteTypeArgs::kWithResponse;
		case winrt::Windows::Devices::Bluetooth::GenericAttributeProfile::GattWriteOption::WriteWithoutResponse:
			return GATTCharacteristicWriteTypeArgs::kWithoutResponse;
		default:
			throw std::bad_cast();
		}
	}

	winrt::Windows::Devices::Bluetooth::GenericAttributeProfile::GattProtectionLevel PeripheralManagerImpl::ArgsToProtectionLevel(const GATTProtectionLevelArgs &level_args)
	{
		switch (level_args)
		{
