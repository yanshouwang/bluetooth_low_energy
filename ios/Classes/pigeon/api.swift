...
class CentralManagerHostApiSetup {
  ...
  static func setUp(binaryMessenger: FlutterBinaryMessenger, api: CentralManagerHostApi?) {
    let authorizeChannel = FlutterBasicMessageChannel(name: "dev.flutter.pigeon.CentralManagerHostApi.authorize", binaryMessenger: binaryMessenger, codec: codec)
    if let api = api {
...
protocol GattCharacteristicHostApi {
  ...
  func write(id: String, value: [UInt8], withoutResponse: Bool, completion: @escaping () -> Void)
  ...
  static func setUp(binaryMessenger: FlutterBinaryMessenger, api: GattCharacteristicHostApi?) {
    ...
    if let api = api {
      writeChannel.setMessageHandler { message, reply in
        ...
        let withoutResponseArg = args[2] as! Bool
        api.write(id: idArg, value: valueArg, withoutResponse: withoutResponseArg) {
          reply(nil)
        }
      }
    } else {
